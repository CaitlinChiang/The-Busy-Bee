...
class Regional_Distributor extends Component {
    state = {
        region: '',
        password: '',
        password_check: true,
    ...
    componentWillMount = _ => {
        ...
        this.setState({ 
            region: location[location.length - 1].replace(/%20/g, " "),
            password: location[location.length - 2]
        })
    }

    componentDidMount = _ => {
        const { region, password } = this.state

        firebase.database().ref('distributors').child('regions').child(region).child('distributor').once('value', snapshot => {
    ...
    provinces_fetch = _ => {
        ...
        firebase.database().ref('locations').once('value', snapshot => {
            snapshot.forEach((snap) => {
                ...
                else if (snap.val().province_region === region) {
                    this.setState({ provinces: this.state.provinces.concat(snap.key) })
                }
            })
        })
    ...
    paymentMethods_fetch = _ => {
        firebase.database().ref('distributors').child('regions').child(this.state.region).child('payment_methods').once('value', snapshot => {
            snapshot.forEach((snap) => {
                var obj = {
    ...
    orders_render = props => {
        ...
        for (let i = 0; i < provinces.length; i++ ) {
            if (provinces[i] === props[1].order_detail[0].order_details.province) {
                return (
                    <tr key={props[0].key}>
    ...
    paymentMethod_add = _ => {
        ...
        if (account_method.trim() !== '' && account_number.trim() !== '') {
            ...
            if (confirm) {
                firebase.database().ref('distributors').child('regions').child(this.state.region).child('payment_methods').child(helpers.timestamp()).update({
...
export default Regional_Distributor